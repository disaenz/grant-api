...
async def list_all_grants(db: AsyncSession = Depends(get_db)):
    logger.debug("Endpoint list_all_grants called")
    grants = await fetch_all(db)
    logger.debug("Fetched %d grants", len(grants))
    return grants


@router.get("/{grant_id}", response_model=GrantResponse)
async def get_one_grant(grant_id: int, db: AsyncSession = Depends(get_db)):
    logger.debug("Endpoint get_one_grant called with grant_id=%d", grant_id)
...
async def create_new_grant( grant_in: GrantCreate, db: AsyncSession = Depends(get_db)):
    logger.debug("Endpoint create_new_grant called with payload: %s", grant_in.model_dump())
    new_id = await add_grant(db, grant_in)
    logger.debug("Created grant with id=%d", new_id)
...
async def update_existing_grant(grant_id: int, grant_in: GrantUpdate, db: AsyncSession = Depends(get_db)):
    logger.debug("Endpoint update_existing_grant called with grant_id=%d payload=%s", grant_id, grant_in.model_dump(exclude_unset=True),)
